...
public class Warehouse {
    ...
    private void addToEnd(int id, String name, int stock, int day, int demand) {
        Product object = new Product(id, name, stock, day, demand);
        sectors[id % 10].add(object);
    }
    ...
    public void purchaseProduct(int id, int day, int amount) {
        ...
        if (!val)
            return;
        if (x.get(i).getStock() - amount < 0)
            return;
        x.get(i).setLastPurchaseDay(day);
        x.get(i).updateStock(-1 * amount);
        x.get(i).updateDemand(amount);
        if (x.get(i) != null) {
            x.swim(i);
            x.sink(i);
        }
    ...
    public void betterAddProduct(int id, String name, int stock, int day, int demand) {
        int index = id%10;
        Sector x = sectors[index];
        int copyIndex = increment(index, sectors.length);
        if (x.getSize() == 5) {
            while(copyIndex!=index) {
                x = sectors[copyIndex];
                if(x.getSize() != 5) {
                    Product object = new Product(id, name, stock, day, demand);
                    sectors[copyIndex].add(object);
                    x.swim(x.getSize());
                    return;
                }
                copyIndex = increment(copyIndex, sectors.length);
